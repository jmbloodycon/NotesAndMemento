...
class NotificationFragment : Fragment(), View.OnClickListener {
    ...
    companion object {  // тут лежит всё статическое
        ...
        fun newInstance(name: String) : NotificationFragment {
    ...
    override fun onStart() {        // метод жизненного цикла. Вызывается, когда фрагмент становится видимым
        ...
        viewAdapter = NotificationAdapter(Notifications.notifications).also {   // и адаптер
            it.listener = object : NotificationAdapter.Listener {
...
//                        .commit()       // выполняем транзакцию
        ...
        recyclerView = layout!!.findViewById<RecyclerView>(R.id.notification_recycler).apply {
    ...
    private fun onAddClick(){
        callback?.onAddNotificationClicked()    // вызываем соответствующий метод активности, чтобы вызвать другой фрагмент
    }
}
